...
import org.jboss.ddoyle.jbpm5.process.audit.JPAWorkingMemoryDbLogger;
...
public class SimpleKSessionService implements KSessionService {
	 ...
	 */
	private static final String GUVNOR_KNOWLEDGE_PKG_URL_PATH = "http://localhost:18080/jboss-brms/rest/packages/org.jboss.ddoyle.poc/binary";
	...
	public StatefulKnowledgeSession getKSession(Integer ksessionId) {
		...
		//Wrap the SKS in a wrapper. This wrapper is used to keep track of loggers registered with the session so they can be closed when the session is disposed.
		...
		registerWorkItemHandlers(wrappedKSession);
	...
	private void initializeLoggers(StatefulKnowledgeSessionWrapper ksessionWrapper) {
		...
		//TODO: This code seriously needs to be cleaned. Referencing properties withoout using constants ...????
...
/**
	 ...
	 * @param ksession
	 */
	private void registerWorkItemHandlers(final StatefulKnowledgeSession ksession) {
		/**
		 * Register our WorkItemHandlers.
		 */
		LOGGER.info("Registering 'Long Running External Process' WorkItemHandler.");
		// Dnamically load the registerd WorkItemHandlers.
		 ...
		 * Apart from the fact that it is discouraged to start threads in a JEE environment, it also doesn't scale very well. Under heavy load, with a
		 * lot of human tasks, the system will create an insane amount of listener threads. Second, when a node crashes (or is just plain
		 * restarted), completion of Human Tasks will not trigger continuation of the process, because there are no threads listening for a
		 * response anymore. This definitely needs to be redesigned. We will base our design on the HornetW WSHumanTaskHandler and our own
	...
	private class StatefulKnowledgeSessionWrapper implements StatefulKnowledgeSession {
		...
		public void dispose() {
			...
			// TODO We could chose to implement some sort of Event that triggers the cleanup of the Sessions from the JPA tables every so often.
		}
